...
interface PlaywrightJsonReport {
  suites: Array<{
    specs: Array<{ tests: Array<{ results: Array<PlaywrightTestResult> }>; title: string }>;
  }>;
...
export function parseResult(stdout: string): RunTestCaseOutput {
  ...
  try {
    ...
    return {
      status: 'failed',
      duration_ms: 0,
      errors: [{ message: 'Falha ao parsear output do Playwright' }],
      artifacts: {},
      testId,
    };
  }
  ...
  if (!firstResult) {
    return {
      status: 'failed',
      duration_ms: 0,
      errors: [{ message: 'Nenhum teste encontrado' }],
      artifacts: {},
      testId,
    };
  }
  ...
  for (const att of firstResult.attachments ?? []) {
    if (att.name === 'screenshot' && att.path)
      artifacts.screenshot = `loomi://artifacts/${testId}/screenshot.png`;
